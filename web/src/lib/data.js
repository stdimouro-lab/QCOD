...
export const LOCAL_KEYS = {
  ...
  // Master Asset List
  masterAssetList: 'qcod-master-asset-list',
  masterAssetListImportStatus: 'qcod-master-asset-list-import-status',
};
...
export function exportQcodBackup() {
  const backup = {
    ...
    masterAssetList: getMasterAssetList(),
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (Array.isArray(backup.masterAssetList)) saveLocalData(LOCAL_KEYS.masterAssetList, backup.masterAssetList);
...
// ---- Master Asset List ----
// The VA's official reference asset file — imported once (or refreshed
// periodically) and compared against QCOD's own scanned/imported inventory.
// This is a comparison dataset, not a mapping tool: AssetWorx still owns
// assigning room/location upstream. QCOD just shows how the scan compares.

export function getMasterAssetList() {
  return loadLocalData(LOCAL_KEYS.masterAssetList, []);
}

export function saveMasterAssetList(assets) {
  saveLocalData(LOCAL_KEYS.masterAssetList, assets);
  saveLocalData(LOCAL_KEYS.masterAssetListImportStatus, {
    lastImportedAt: new Date().toISOString(),
    count: assets.length,
  });
  recordAuditEntry(buildAuditEntry({
    action: 'master_asset_list_imported', entityType: 'master_asset_list', entityId: 'master-list',
    newValue: assets.length, notes: `Imported ${assets.length} master asset record(s)`,
  }));
}

export function getMasterAssetListImportStatus() {
  return loadLocalData(LOCAL_KEYS.masterAssetListImportStatus, { lastImportedAt: '', count: 0 });
}
